...
- **CRD cross-check: the third filter, and the one the other two cannot replace.** `ownerReferences` is free and catches operator-*created* objects (cert-manager's Certificate → its Secret); it does nothing for an operator that *references* a hand-made ConfigMap by name, which is the common shape and leaves no trace in any native field. So candidates that survive the native scan are matched against every string key and string leaf of every CR in the cluster. Bare string equality on purpose: an operator's schema is its own, so `spec.configMapRef.name` / `spec.tls.secretName` / `spec.template.volumes[].configMap.name` cannot be enumerated ahead of time. The asymmetry decides the design — a coincidental match keeps an object that was in fact unused, a missed reference is a delete recommendation for live config.
  - **Two-phase, so the expensive half is skipped in a clean cluster.** `collectFindings()` → derive the candidate names → only then `crossCheckCustomResources()`. Nothing to disprove means no CRD pass at all. CRs are folded into the mentions map per batch of 8 rather than collected, so a big cluster's CRs never all sit in memory at once.
  - **Namespace scoping follows the CR's own scope:** a namespaced CR only protects names in its own namespace; a cluster-scoped one (ClusterIssuer, a Prometheus rule) protects the name everywhere it is a candidate, because it can legitimately point at any namespace. Over-keeping, again, beats over-deleting.
  - **An unreadable CRD rewrites the note, it does not get swallowed.** `crossCheck.crdsUnreadable` names them and the wording drops from "all N CRD kinds were scanned" to "N could NOT be read … say the cross-check was partial". `managedFields` is the one subtree skipped — mechanical audit trail, nothing references anything from there. Cap is 80 CRD kinds; anything past it is *reported* as unreadable, never silently cut.
  - Needs `rbac.readAllCustomResources` in the chart (0.3.0+): `get`/`list` on `apiGroups ["*"]`. Read-only, and the same ClusterRole already granted cluster-wide Secret read.
